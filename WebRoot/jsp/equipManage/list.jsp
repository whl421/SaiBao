...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >设备管理</span>
	</nav> 
							...
							<div class="form-group">
								<label class="date-label" ></label>
								  <select class="selectpicker form-control" data-done-button="true"
								   title="状态选择" id="estate" name="tablesearch">
							...
							<div class="btn-groups hidden-xs" id="exampleToolbar" role="group">
								<c:if test="${button.add!=null}">
									<div id="add" class="btn-group">
										<a href="javascript:PopupModalView(null,'refresh','设备管理>>新增','60%','500px','${pageContext.request.contextPath}/equip/addForm.action');"  class="btn btn-primary btn-sm">
	...
	<script type="text/javascript">
		...
		*/
		function updatetable(index){
			var _url="${pageContext.request.contextPath}/equip/updateForm.action?tablejson="+index;
			PopupModalView(null,'refresh','设备管理>>修改','60%','500px',_url);
		}
		/**
		* 查看
		*/
		function infotable(index){
			var _url="${pageContext.request.contextPath}/equip/infoForm.action?tablejson="+index;
			PopupModalView(null,'refresh','设备管理>>查看','60%','500px',_url);
		}
		/**
		*删除数据
		*/
		function deletetable(){
			...
			if (rows.length > 0) {
				...
				var durl="${pageContext.request.contextPath}/equip/delete.action";
		...
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/equip/list.action",
				//height: "650",			 //行高，如果没有设置height属性，
				uniqueId: "equipid",          //每一行的唯一标识，一般为主键列
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			// 设置隐藏列
			$table.bootstrapTable('hideColumn', 'equipid');
			...
			function estate(value,row,index) {
				var str = "";
				if(row.estate == "0") {
					str = "禁用";
				} else if(row.estate == "1") {
					str = "启用";
			...
			function rowbutton(value,row,index){
				...
				if("${button.update}"&&"${button.update}"!=null&&"${button.update}"!=""){
					bt1 += "<button type='button' onclick='updatetable("+row.equipid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-wrench'></i>修改</button>&nbsp;";
				}
				if("${button.info}"&&"${button.info}"!=null&&"${button.info}"!=""){
					bt1 += "<button type='button' onclick='infotable("+row.equipid+")' class='btn btn-info btn-sm'><i class='fa fa-search'></i>查看</button>&nbsp;";
				}
				if(bt1 == "") {
					bt1 = "----";
				}
