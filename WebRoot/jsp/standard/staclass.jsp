...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >标准分类</span>
	</nav>
...
<script type="text/javascript">
	...
	$.getJSON("${pageContext.request.contextPath}/standard/staclasstree.action"+"?rundd="+new Date().getTime(),function(datas){
		 $('#treeview').treeview({
			 color: "#5F6368",
					...
					<div class="example-wrap">
							<div class="form-inline">
								 <div class="form-group">
									 ...
									 <input type="text" class="form-control" id="sclassname" name="tablesearch" placeholder="分类名称">
								 </div>
								...
								<c:if test="${button.add!=null}">
									...
									<script type="text/javascript">
										function addthis(){
											...
											PopupModalView(id,'refresh','日常奖扣>>分类管理','800px','500px','${pageContext.request.contextPath}/standard/staclassadd.action');
										}
									</script>
								</c:if>
    ...
    <script type="text/javascript">
		...
		*/
		function updateGl(index){
			var _url="${pageContext.request.contextPath}/standard/staclassload.action?tablejson="+index;
			PopupModalView(null,'refresh','日常奖扣>>分类管理','800px','500px',_url);
		}
		/**
		*删除数据
		*/
		function deletetable(){
			...
			if (rows.length > 0) {
				...
				//alert(JSON.stringify(rows));
				 var list=new Array();
				 for(row in rows){
				 	list[row]=rows[row].sclassid;
				 }
				 var durl="${pageContext.request.contextPath}/standard/staclassdelete.action";
...
// 			alert(JSON.stringify(row1));
			var _url="${pageContext.request.contextPath}/standard/staclassload.action?tablejson="+index;
			PopupModalView(null,'refresh','日常奖扣>>分类管理','800px','300px',_url);
		}
	/**
	表格加载
	*/
	...
	$(function() {
		 ...
		 $table= $('#exampleTablePagination').bootstrapTable({
		    url: "${pageContext.request.contextPath}/standard/staclasslist.action",
			uniqueId: "sclassid",          //每一行的唯一标识，一般为主键列
...
//                     //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
		  ...
		  // 设置隐藏列
		  $table.bootstrapTable('hideColumn', 'sclassid');
		  $table.bootstrapTable('hideColumn', 'parentid');
		  //下拉级联
		  /**
		  *可以设置按钮
		  *formatter:peronsex
		  */
		  function rowbutton(value,row,index){
			...
			if("${button.update}"&&"${button.update}"!=null&&"${button.update}"!=""){
				bt1 += "<button type='button' onclick='updateGl("+row.sclassid+")' class='btn btn-info btn-sm'><i class='fa fa-wrench'></i>&nbsp;修改</button>";
			}
			if(bt1 == "") {
				bt1 = "----";
			}
