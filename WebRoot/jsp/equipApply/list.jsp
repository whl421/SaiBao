...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >设备预约申请</span>
	</nav> 
				...
				<div class="col-sm-12">
					<div class="example-wrap">
						<div class="form-inline">
							...
							<div class="form-group">
								<input type="hidden" class="form-control" id="applystate" name="tablesearch" placeholder="状态" value="">
							</div>
							<div class="form-group ">
							...
							<div class="btn-groups hidden-xs" id="exampleToolbar" role="group">
								<c:if test="${button.add!=null}">
									<div id="add" class="btn-group">
										<a href="javascript:PopupModalView(null,'refresh','设备预约申请>>预约','60%','500px','${pageContext.request.contextPath}/equipApply/addForm.action');"  class="btn btn-primary btn-sm">
											<i class="glyphicon glyphicon-plus" aria-hidden="true"></i>&nbsp;预约
										</a>
									</div>
								</c:if>
	...
	<script type="text/javascript">
		...
		*/
		function infotable(index){
			var _url="${pageContext.request.contextPath}/equipApply/infoForm.action?tablejson="+index;
			PopupModalView(null,'refresh','设备预约申请>>查看','60%','80%',_url);
		}
		/**
		*删除数据
		*/
		function deletetable(){
			...
			if (rows.length > 0) {
				...
				var durl="${pageContext.request.contextPath}/equipApply/delete.action";
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			function equipapolycode(value,row,index) {
				var str = "";
				if(row.applystate == "3") {
			...
			function rowbutton(value,row,index){
				...
				if("${button.update}"&&"${button.update}"!=null&&"${button.update}"!=""){
					bt1 += "<button type='button' onclick='updatetable("+row.equipapplyid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-wrench'></i>修改</button>";
				}
				if("${button.info}"&&"${button.info}"!=null&&"${button.info}"!=""){
					bt1 += "&nbsp;<button type='button' onclick='infotable("+row.equipapplyid+")' class='btn btn-info btn-sm'><i class='fa fa-search'></i>查看</button>&nbsp;";
				}
				if(bt1 == "") {
					bt1 = "----";
				}
