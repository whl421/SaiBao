...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >设备预约审核</span>
	</nav> 
					...
					<div class="example-wrap">
						<div class="form-inline">
							<div class="form-group">
								...
								<input type="text" class="form-control" id="equipapolycode" name="tablesearch" placeholder="申请单号">
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
								<input type="text" class="form-control" id="equipcode" name="tablesearch" placeholder="设备编号">
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
								<input type="text" class="form-control" id="equipname" name="tablesearch" placeholder="设备名称">
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
							...
							<div class="form-group" style="width:220px" >
								<input type="text" id="applydate" class="form-control" style="width:210px" name="tablesearch" placeholder="申请日期选择">
							</div>
							<div class="form-group" style="width:220px" >
								<input type="text" id="usetime" class="form-control" style="width:210px" name="tablesearch" placeholder="使用时间选择">
							</div>
							<div class="form-group">
								<input type="hidden" class="form-control" id="applystate" name="tablesearch" placeholder="状态" value="1">
							</div>
							<div class="form-group ">
						...
						<div class="example">
							<div class="btn-groups hidden-xs" id="exampleToolbar" role="group">
								<c:if test="${button.batchpass!=null}">
									<div id="batchpass" class="btn-group">
										<button type="button" onclick="batchpass()" class="btn btn-primary btn-sm">
											<i class="glyphicon glyphicon-ok" aria-hidden="true"></i>&nbsp;批量通过
										</button>
									</div>
								</c:if>
								<c:if test="${button.dismissal!=null}">
									<div id="dismissal" class="btn-group">
										<button type="button" onclick="dismissal()" class="btn btn-warning btn-sm">
											<i class="glyphicon glyphicon-remove" aria-hidden="true"></i>&nbsp;批量驳回
										</button>
									</div>
								</c:if>
							</div>
	...
	<script type="text/javascript">
		...
		*/
		function check(index){
			var _url="${pageContext.request.contextPath}/equipApply/checkForm.action?tablejson="+index;
			PopupModalView(null,'refresh','设备预约审核>>审核','60%','80%',_url);
		}
		/**
		*批量通过
		*/
		function batchpass(){
			...
			if (rows.length > 0) {
				var list=new Array();
				for(row in rows){
				 	list[row]=rows[row].equipapplyid;
				}
				var durl="${pageContext.request.contextPath}/equipApply/batchpass.action";
		...
		*/
		function dismissal(){
			...
			if (rows.length > 0) {
				...
				var durl="${pageContext.request.contextPath}/equipApply/dismissal.action";
				var ajax = doajax(durl,list);
				if(ajax){
				 	$table.bootstrapTable('selectPage', 1);
					$table.bootstrapTable('refresh');
		...
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/equipApply/list.action",
				//height: "650",			 //行高，如果没有设置height属性，
				uniqueId: "equipapplyid",          //每一行的唯一标识，一般为主键列
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			// 设置隐藏列
			$table.bootstrapTable('hideColumn', 'equipapplyid');
			//显示省略号
			...
			function equipcode(value,row,index) {
				var str = "";
				if(row.applystate == "0") {
			...
			function rowbutton(value,row,index){
				...
				if("${button.check}"&&"${button.check}"!=null&&"${button.check}"!=""){
					bt1 += "<button type='button' onclick='check("+row.equipapplyid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-wrench'></i>审核</button>";
				}
				if(bt1 == "") {
					bt1 = "----";
				}
