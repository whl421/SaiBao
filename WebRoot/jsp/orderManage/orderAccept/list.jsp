...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >委托单接收</span>
	</nav> 
	...
	<script type="text/javascript">
		...
		*/
		function updatetable(index){
			var _url="${pageContext.request.contextPath}/orderAccept/acceptForm.action?tablejson="+index;
			PopupModalView(null,'refresh','委托单接收>>接收','60%','550px',_url);
		}
		/**
		* 任务发起
		*/
		function taskfq(index){
			var _url="${pageContext.request.contextPath}/orderAccept/taskFqForm.action?tablejson="+index;
			PopupModalView(null,'refresh','委托单接收>>任务发起','60%','500px',_url);
		}
		/**
		表格加载
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/orderAccept/list.action",
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			function liststate(value,row,index) {
				if(row.liststate == "1") return "待确认";
				if(row.liststate == "2") return "接收";
				if(row.liststate == "3") return "拒绝";
				if(row.liststate == "4") return "任务发起";
				if(row.liststate == "5") return "任务接收";
				if(row.liststate == "6") return "任务拒绝";
			}
			function rowOrderCode(value,row,index) {
				var str = "";
				if(row.liststate == "3" || row.liststate == "6") {
	...
	//			alert(index);
				...
				if(row.liststate == "1") {
					if("${button.update}"&&"${button.update}"!=null&&"${button.update}"!=""){
						bt1 += "<button type='button' onclick='updatetable("+row.orderid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-wrench'></i>&nbsp;接收</button>&nbsp;";
					}
				}
				if(row.liststate == "2" || row.liststate == "6") {
					if("${button.taskfq}"&&"${button.taskfq}"!=null&&"${button.taskfq}"!=""){
						bt1 += "<button type='button' onclick='taskfq("+row.orderid+")' class='btn btn-success btn-sm'style='width:80px;'><i class='glyphicon glyphicon-wrench'></i>&nbsp;任务发起</button>&nbsp;";
					}
				}
				if(bt1 == "") {
					bt1 = "----";
