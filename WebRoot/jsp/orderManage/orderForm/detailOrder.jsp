...
<html>
	...
	<body class="gray-bg" >
		<div class="wrapper wrapper-content animated fadeInRight">
			<form id="myform"   enctype="multipart/form-data">
				<div class="ibox-content col-sm-12">
					...
					<!-- <div class="col-sm-12" >
						<div class=" form-group col-sm-12 ">
							<label  class="date-label col-sm-2"><span style="color:red">*</span>确认</label>
							<div class="col-sm-4 ">
								<div class="form-control col-sm-8"  > 
								 <label class="radio-inline">
									<input type="radio"  value="2" name="ostate" id="ostate" checked >接收
								 </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								 <label class="radio-inline">
									<input type="radio"  value="3" name="ostate" id="ostate" >关闭
								 </label>	
								</div>
							</div>	
							<label  class="date-label col-sm-2">报价</label>
							<div class="col-sm-4 ">
								<input type="text" class="form-control" value="${map.orderprice }"
									null="no" id="orderprice" name="orderprice" >
							</div>	
						</div>
					</div>
					<div class="col-sm-12 ">
						<div class="form-group col-sm-12">
							<label  class="date-label col-sm-2">确认说明</label>
							<div class="col-sm-10">
								<textarea  rows="2" class="form-control" 
								  id="qrmemo"  name="manageIntegral" ></textarea >
							</div> 
						</div>
					</div> -->
		...
		<script type="text/javascript">
			$(function(){
				//数字验证
				$.each($("#compnum"),function(index,obj){
			...
			$("#submitbtn").click(function(){
				var taskresult = document.getElementById("taskresult").value;
				var pffs = document.getElementById("pffs").value;
				if(pffs == "2" && taskresult == "") {
					alert("完成件数不能为空");
					return false;
				}
				...
				$.ajax({
					type: "post",
					url: "${pageContext.request.contextPath}/daytask/hbsubmit.action",
					dataType: "json",
					data:datas,
					cache: false,
					processData: false,
			...
			function setadd(aobj){
				...
				var dayid = $("#dayid").val();
				var daycomid = $("#daycomid").val();
				var taskresult = $("#taskresult").val();
				var taskstate = $("input:radio:checked").val();
				var data = {
					"dayid" : dayid,
					"daycomid" : daycomid,
				...
				$.ajax({
					url : '/edusoft/daytask/hbsubmit.action?rundd='+new Date().getTime(),
					type : 'post',
					data : {tablejson:str},
					dataType : "json",
					async : false,
