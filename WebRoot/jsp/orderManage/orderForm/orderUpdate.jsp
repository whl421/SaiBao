...
<html>
	...
	<body class="gray-bg" >
					...
					<div class="col-sm-12 form-group">
						...
						<div  class="col-sm-10">
							<textarea rows="2" class="form-control col-sm-10"  null="yes" id="khname" name="khname" 
								style="width:100%" >${map.khname}</textarea>
						</div>
					</div>
					<div class="col-sm-12 form-group">
						...
						<div class="col-sm-10">
							<input type="file" class="" null="no" 
								multiple="multiple" name="file" id="file" onclick="addinput();" style="padding-bottom: 10px;" >
							<!-- <input type="button" name="button" value="添加附件" onclick="addinput();" /> -->
							<!-- <input type="button" name="button" value="删除所有" onclick="deleteinput();" /> -->
							<span id="upload" >
								<c:forEach items="${fjmap }" var="fj">
									<a style="display: inline-block;color:blue;" href="../order/downLoad.action?id=${fj.id}">${fj.sourcefile }</a><br/>
								</c:forEach>
							</span>
						</div> 
					</div>
		...
		$("#submitbtn").click(function(){
			...
			$.ajax({
				type: "post",
				url: "${pageContext.request.contextPath}/order/updateinfo.action",
				dataType: "json",
				data:datas,
				cache: false,
				processData: false,
