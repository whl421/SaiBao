...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >委托记录查询</span>
	</nav> 
	...
	<script type="text/javascript">
		...
		*/
		function infotable(index){
			var _url="${pageContext.request.contextPath}/orderRec/infoForm.action?tablejson="+index;
			PopupModalView(null,'refresh','委托单记录>>查看','60%','80%',_url);
		}
		/**
		表格加载
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/orderRec/list.action",
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			//状态 1新委托  2已接收 3已关闭4已推送 5已完成
			function ostate(value,row,index) {
				if(row.ostate == "1") return "新委托";
				if(row.ostate == "2") return "已接收";
				if(row.ostate == "3") return "已关闭";
				if(row.ostate == "4") return "已推送";
				if(row.ostate == "5") return "已完成";
			}
	...
	//			alert(index);
				...
				if("${button.info}"&&"${button.info}"!=null&&"${button.info}"!=""){
					bt1 += "<button type='button' onclick='infotable("+row.orderid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-wrench'></i>&nbsp;查看</button>";
				}
				if(bt1 == "") {
					bt1 = "----";
				}
