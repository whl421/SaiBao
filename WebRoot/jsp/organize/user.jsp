...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >人员管理</span>
	</nav>
...
<script type="text/javascript">
	...
	$.getJSON("${pageContext.request.contextPath}/userinfo/usertree.action"+"?rundd="+new Date().getTime(),function(datas){
		 $('#treeview').treeview({
			 color: "#5F6368",
...
<!-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>树形结束>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>					 -->			
					...
					<div class="example-wrap">
							<div class="form-inline">
								<div class="form-group ">
									<a id="type0" href="javascript:;" onclick="typeload('type0','1')" class="btn btn-default ">在&nbsp;&nbsp;&nbsp;职</a>
								</div>
								<div class="form-group ">
									<a id="type1" href="javascript:;" onclick="typeload('type1','0')" class="btn btn-default ">离&nbsp;&nbsp;&nbsp;职</a>&nbsp;&nbsp;
								</div>
								<div class="form-group">
									<label  class="date-label"></label>
									<input type="text" class="form-control" id="username" name="tablesearch" placeholder="姓名">
								</div>
								<input type="hidden" id="state" name="tablesearch" value="1">
								<div class="form-group">
									<label  class="date-label"></label>
								</div>
								 <div class="form-group ">
								 ...
								 <div class="form-group ">
									&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="isparent"  onclick="isparent()" name="tablesearch" />统计子部门人员
								</div>
								<input type="hidden" id="parent" name="tablesearch" value="0">
							</div>	
						<div class="example">
							 	...
							 	<c:if test="${button.add!=null}">
									...
									<script type="text/javascript">
										function addthis(){
											...
											PopupModalView(id,'refresh','组织机构>>人员管理','50%','45%','${pageContext.request.contextPath}/userinfo/useradd.action');
										}
									</script>
								</c:if>
    ...
    <script type="text/javascript">
		...
		document.getElementById("type0").style.backgroundColor="PaleTurquoise";
		function typeload(divid1,jftype){
			document.getElementById("type0").style.backgroundColor="white";
			document.getElementById("type1").style.backgroundColor="white";
			
			document.getElementById(divid1).style.color="black";
			document.getElementById(divid1).style.backgroundColor="PaleTurquoise";
			
			document.getElementById("state").value = jftype;
		}
		
    	//是否统计子部门
    	function isparent(){
    		var a = document.getElementById("isparent");
    		if(a.checked){
		...
		*/
		function updateGl(index){
			var _url="${pageContext.request.contextPath}/userinfo/userload.action?tablejson="+index;
			PopupModalView(null,'refresh','组织机构>>人员管理','50%','45%',_url);
		}
		/**
		*删除数据
		*/
		function deletetable(){
			...
			if (rows.length > 0) {
				...
				//alert(JSON.stringify(rows));
				 var list=new Array();
				 for(row in rows){
				 	list[row]=rows[row].userid;
				 }
				 var durl="${pageContext.request.contextPath}/userinfo/userdelete.action";
...
// 			alert(JSON.stringify(row1));
			var _url="${pageContext.request.contextPath}/userinfo/userload.action?tablejson="+index;
			PopupModalView(null,'refresh','组织机构>>人员查看','1200px','850px',_url);
		}
	/**
	表格加载
	*/
	...
	$(function() {
		 ...
		 $table= $('#exampleTablePagination').bootstrapTable({
		    url: "${pageContext.request.contextPath}/userinfo/userlist.action",
		  ...
		  */
		  ...
		  function rowbutton(value,row,index){
			...
			if("${button.update}"&&"${button.update}"!=null&&"${button.update}"!=""){
				bt1 += "<button type='button' onclick='updateGl("+row.userid+")' class='btn btn-info btn-sm'><i class='fa fa-wrench'></i>&nbsp;修改</button>";
			}
			if(bt1 == "") {
				bt1 = "----";
			}
