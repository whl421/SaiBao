...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >部门管理</span>
	</nav>
...
<script type="text/javascript">
	...
	$.getJSON("${pageContext.request.contextPath}/dept/depttree.action"+"?rundd="+new Date().getTime(),function(datas){
		 $('#treeview').treeview({
			 color: "#5F6368",
					...
					<div class="example-wrap">
							<div class="form-inline">
								 <div class="form-group">
									 ...
									 <input type="text" class="form-control" id="departname" name="tablesearch" placeholder="部门名称">
								 </div>
							 	...
							 	<c:if test="${button.add!=null}">
									...
									<script type="text/javascript">
										function addthis(){
											...
											PopupModalView(id,'refresh','组织机构>>部门管理','70%','60%','${pageContext.request.contextPath}/dept/deptadd.action');
										}
									</script>
								</c:if>
    ...
    <script type="text/javascript">
		...
		*/
		function updateGl(index){
			var _url="${pageContext.request.contextPath}/dept/deptload.action?tablejson="+index;
			PopupModalView(null,'refresh','组织机构>>部门管理','800px','400px',_url);
		}
	/**
	*删除数据
	*/
	function deletetable(){
		...
		if (rows.length > 0) {
			...
			var list=new Array();
			for(row in rows){
				list[row]=rows[row].departid;
			}
			
			if(deleteYZ(list)){
				var durl="${pageContext.request.contextPath}/dept/deptdelete.action";
		...
		function deleteYZ(ids){	
			var flag = false;
			if(ids!=null){
			ids = JSON.stringify(ids);
				$.ajax({
					url : '${pageContext.request.contextPath}/dept/deptdeleteYZ.action?rundd='+new Date().getTime(),
					type : 'post',
				 	data : {tablejson:ids},
			        dataType : "json",
				    async : false,
...
// 			alert(JSON.stringify(row1));
			var _url="${pageContext.request.contextPath}/dept/deptload.action?tablejson="+index;
			PopupModalView(null,'refresh','组织机构>>部门管理','800px','300px',_url);
		}
/**
表格加载
*/
...
$(function() {
		 ...
		 $table= $('#exampleTablePagination').bootstrapTable({
		    url: "${pageContext.request.contextPath}/dept/deptlist.action",
			uniqueId: "departid",          //每一行的唯一标识，一般为主键列
...
//                     //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
		  ...
		  // 设置隐藏列
		  $table.bootstrapTable('hideColumn', 'departid');
		  //下拉级联
		  /**
		  *可以设置按钮
		  *formatter:peronsex
		  */
		  function rowbutton(value,row,index){
			...
			if("${button.update}"&&"${button.update}"!=null&&"${button.update}"!=""){
				bt1 += "<button type='button' onclick='updateGl("+row.departid+")' class='btn btn-info btn-sm'><i class='fa fa-wrench'></i>&nbsp;修改</button>";
			}
			if(bt1 == "") {
				bt1 = "----";
			}
