...
<body class="gray-bg">
	<nav class="navigation" >
		<span class="title" >材料库存管理</span>
		<span class="title" >-</span>
		<span class="titlethis" >出入库明细</span>
	</nav> 
							...
							<div class="form-group">
								<label class="date-label" ></label>
								  <select class="selectpicker form-control" data-done-button="true"
								   title="操作类型" id="billtype" name="tablesearch">
									<option value="">全部</option>
									<option value="1">采购入库</option>
									<option value="2">领用出库</option>
									<option value="3">退料入库</option>
								</select>
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
								<input type="text" class="form-control" id="createtime" name="tablesearch" placeholder="操作日期">
							</div>
							<div  class="form-group">
								<input type="hidden" class="form-control" id="storclid" name="tablesearch" placeholder="库存ID" value="${storclid}">
							</div>
							<div class="form-group ">
		...
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/matStor/billList.action",
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			// 设置隐藏列
			$table.bootstrapTable('hideColumn', 'billid');
			function billtype(value,row,index){
				var str = "";
				if(row.billtype == "1") {
					str = "采购入库";
