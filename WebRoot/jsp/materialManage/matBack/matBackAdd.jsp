...
<html>
	...
	<body class="gray-bg" >
			...
			<div class="ibox-content col-sm-12">
				...
				<div class="col-sm-12 " style="margin-top: 10px;">
					<div class="form-group col-sm-12">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" readonly="readonly"
							  id="backusername"  name="backusername" value="${map.backusername}" />
						</div> 
						<label  class="date-label col-sm-2">退回时间</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" readonly="readonly"
							  id="backtime"  name="backtime" value="${map.backtime}" />
						</div>
					</div>
				</div>
				<div class="col-sm-12" style="margin-top: 10px;">
		...
		*/
		...
		var $table;
		$(function() {
			var options = {editFiled:"backnum"}
			$("#matList").bootstrapTable('destroy');
			$table= $('#matList').bootstrapTable({
				url: "${pageContext.request.contextPath}/matBack/clApplyList.action",//
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
				...
				onDblClickCell : function(field,value,row,$element){
					...
					if(field == options.editFiled){
						...
						$("#inputCell").blur(function(){
							...
							if(parseFloat(row.applynum) < newValue) {
								alert("退回数量不能大于申请数量");
								$table.bootstrapTable('updateCell', {
									index: upIndex,
									field:field,
		...
		function setadd(applystate){
			...
			if(rows.length > 0) {
				var list = new Array();
				for(row in rows){
					var backnum = rows[row].backnum;
					if(backnum != null && backnum != "" && backnum != "0" && backnum != "null") {
						j++;
					}
				}
			}
			if(j == 0) {
				alert("退回数量不能全为空或者全为0");
				return;
			}
			var sublist = JSON.stringify(rows);
			$.ajax({
				url : '${pageContext.request.contextPath}/matBack/save.action?rundd='+new Date().getTime(),
				type : 'post',
