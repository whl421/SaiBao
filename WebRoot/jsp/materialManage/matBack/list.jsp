...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >材料退回申请</span>
	</nav> 
				...
				<div class="col-sm-12">
					<div class="example-wrap">
						<div class="form-inline">
							...
							<div class="form-group">
								<label  class="date-label"></label>
								<input type="hidden" class="form-control" id="applystate" name="tablesearch" placeholder="状态" value="2">
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
	...
	<script type="text/javascript">
		...
		*/
		function infotable(index){
			var _url="${pageContext.request.contextPath}/matBack/infoForm.action?tablejson="+index;
			PopupModalView(null,'refresh','材料退回申请>>退回记录','60%','80%',_url);
		}
		/**
		* 退回
		*/
		function backtable(index){
			var _url="${pageContext.request.contextPath}/matBack/backForm.action?tablejson="+index;
			PopupModalView(null,'refresh','材料退回申请>>退回','60%','80%',_url);
		}
		/**
		表格加载
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/matBack/list.action",
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			function rowbutton(value,row,index){
				var bt1 = "";
				var thflag = row.thflag;
				if(thflag == null || thflag == "" || thflag == "0") {
					if("${button.back}"&&"${button.back}"!=null&&"${button.back}"!=""){
						bt1 += "<button type='button' onclick='backtable("+row.clapplyid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-triangle-left'></i>退回</button>&nbsp;";
					}
				} else {
					if("${button.info}"&&"${button.info}"!=null&&"${button.info}"!=""){
						bt1 += "<button type='button' onclick='infotable("+row.clapplyid+")' class='btn btn-info btn-sm'><i class='fa fa-search'></i>退回记录</button>&nbsp;";
					}
				}
				if(bt1 == "") {
					bt1 = "----";
