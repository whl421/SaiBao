...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >材料申请审核</span>
	</nav> 
					...
					<div class="example-wrap">
						<div class="form-inline">
							<div class="form-group">
								...
								<input type="text" class="form-control" id="applycode" name="tablesearch" placeholder="申请单号">
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
								...
								<c:if test="${button.batchpass!=null}">
									<div id="batchpass" class="btn-group">
										<button type="button" onclick="batchpass()" class="btn btn-primary btn-sm">
											<i class="glyphicon glyphicon-ok" aria-hidden="true"></i>&nbsp;批量同意
										</button>
									</div>
								</c:if>
								<c:if test="${button.batchrefuse!=null}">
									<div id="batchrefuse" class="btn-group">
										<button type="button" onclick="batchrefuse()" class="btn btn-warning btn-sm">
											<i class="glyphicon glyphicon-remove" aria-hidden="true"></i>&nbsp;批量拒绝
										</button>
									</div>
								</c:if>
							</div>
	...
	<script type="text/javascript">
		...
		*/
		function infotable(index){
			var _url="${pageContext.request.contextPath}/matCheck/infoForm.action?tablejson="+index;
			PopupModalView(null,'refresh','材料申请审核>>查看','60%','80%',_url);
		}
		/**
		* 审核
		*/
		function check(index){
			var _url="${pageContext.request.contextPath}/matCheck/checkForm.action?tablejson="+index;
			PopupModalView(null,'refresh','材料申请审核>>审核','60%','80%',_url);
		}
		/**
		*批量同意
		*/
		function batchpass(index){
			var rows = $table.bootstrapTable('getSelections');
			if (rows.length > 0) {
				var list=new Array();
				for(row in rows){
				 	list[row]=rows[row].clapplyid;
				}
				var durl="${pageContext.request.contextPath}/matCheck/batchpass.action";
		...
		*/
		function batchrefuse(){
			...
			if (rows.length > 0) {
				...
				var durl="${pageContext.request.contextPath}/matCheck/batchrefuse.action";
				var ajax = doajax(durl,list);
				if(ajax){
				 	$table.bootstrapTable('selectPage', 1);
					$table.bootstrapTable('refresh');
		...
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/matCheck/list.action",
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			function rowbutton(value,row,index){
				...
				if("${button.check}"&&"${button.check}"!=null&&"${button.check}"!=""){
					bt1 += "<button type='button' onclick='check("+row.clapplyid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-user'></i>审核</button>&nbsp;";
				}
				if(applytype == "1") {
