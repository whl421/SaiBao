...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >客户信息</span>
	</nav>
					...
					<div class="example-wrap">
							<div class="form-inline">
								 <div class="form-group">
									 ...
									 <input type="text" class="form-control" id="cusname" name="tablesearch" placeholder="客户名称">
								 </div>
							 ...
							 <div class="btn-groups hidden-xs" id="exampleToolbar" role="group">
								<c:if test="${button.add!=null}">
									<div id="add" class="btn-group">
										<a href="javascript:PopupModalView(null,'refresh','客户管理>>客户信息','50%','40%','${pageContext.request.contextPath}/cus/cusadd.action');"  class="btn btn-primary btn-sm">
    ...
    <script type="text/javascript">
		...
		*/
		function updateGl(index){
			...
			PopupModalView(null,'refresh','客户管理>>客户信息','70%','95%',_url);
		}
		/**
		*重置密码
		*/
		function updatepwd(index){
			var durl="${pageContext.request.contextPath}/cus/cusupdatepwd.action";
		...
		*/
		function deletetable(index){
			var durl="${pageContext.request.contextPath}/cus/cusdelete.action";
			var ajax=doajax(durl,index);
			if(ajax){
			 	$table.bootstrapTable('selectPage', 1);
				$table.bootstrapTable('refresh');
		...
		*初始化权限
		*/
		function cshrole(index){
			var durl="${pageContext.request.contextPath}/cus/roleadd.action";
			var ajax=doajax(durl,index);
			if(ajax){
			 	$table.bootstrapTable('selectPage', 1);
				$table.bootstrapTable('refresh');
...
$(function() {
		 ...
		 $table= $('#exampleTablePagination').bootstrapTable({
		    url: "${pageContext.request.contextPath}/cus/cuslist.action",
...
//                     //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
		  ...
		  // 设置隐藏列
		  $table.bootstrapTable('hideColumn', 'cusid');
		...
		function rowbutton(value,row,index){
			if(row.state == "0") {
				var bt1 = "";
				if("${button.delete}"&&"${button.delete}"!=null&&"${button.delete}"!=""){
					bt1 += "<button type='button' onclick='deletetable("+row.cusid+")' class='btn btn-warning btn-sm'><i class='glyphicon glyphicon-trash'></i>&nbsp;删除</button>&nbsp;";
				}
				if("${button.updatepwd}"&&"${button.updatepwd}"!=null&&"${button.updatepwd}"!=""){
					bt1 += "<button type='button' onclick='updatepwd("+row.cusid+")' class='btn btn-info btn-sm'><i class='fa fa-wrench'></i>&nbsp;重置密码</button>&nbsp;";
				}
				if("${button.state}"&&"${button.state}"!=null&&"${button.state}"!=""){
					bt1 += "<button type='button' onclick='cshrole("+row.cusid+")' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-user'></i>&nbsp;审核确认</button>";
				}
				return bt1;
			} else {
				var bt1 = "";
