...
<body class="gray-bg">
	<nav class="navigation" >
		...
		<span class="titlethis" >任务跟踪查询</span>
	</nav> 
				...
				<div class="col-sm-12">
					<div class="example-wrap">
						<div class="form-inline">
							...
							<div class="form-group">
								<label  class="date-label"></label>
								<input type="text" class="form-control" id="ordercode" name="tablesearch" placeholder="委托单号">
							</div>
							<div class="form-group">
								<label  class="date-label"></label>
								<input type="text" class="form-control" id="ordername" name="tablesearch" placeholder="委托单名称">
							</div>
	...
	<script type="text/javascript">
		...
		*/
		function info(index){
			var _url="${pageContext.request.contextPath}/taskCl/taskProcessForm.action?tablejson="+index;
			PopupModalView(null,'refresh','任务跟踪查询>>任务过程','60%','70%',_url);
		}
		/**
		表格加载
		*/
		...
		$(function() {
			...
			$table= $('#exampleTablePagination').bootstrapTable({
				url: "${pageContext.request.contextPath}/taskCl/trackList.action",
	...
	//              //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			// 设置隐藏列
			$table.bootstrapTable('hideColumn', 'taskid');
			function taskstate(value,row,index) {
				if(row.taskstate == "1") return "执行中";
				if(row.taskstate == "2") return "已完成";
			}
			...
			function rowbutton(value,row,index){
				...
				if("${button.info}"&&"${button.info}"!=null&&"${button.info}"!=""){
					bt1 += "<button type='button' onclick='info("+row.taskid+")' class='btn btn-success btn-sm'style='width:80px;'><i class='glyphicon glyphicon-wrench'></i>任务过程</button>&nbsp;";
				}
				if(bt1 == "") {
					bt1 = "----";
				}
