	...
	<body class="gray-bg" >
		<div class="wrapper wrapper-content animated fadeInRight">
			...
			<div class="ibox-content col-sm-12">
				...
				<div class="col-sm-12 ">
					<div class="form-group col-sm-12">
						<label  class="date-label col-sm-2 ">上一阶段完成人</label>
						<div class="col-sm-4">
						...
						<label  class="date-label col-sm-2"></label>
						<div class="col-sm-4">
					...
					<div class="form-group col-sm-12">
						...
						<div class="col-sm-10">
							<textarea rows="2" class="form-control" 
								null="no" id="wcmemo" name="wcmemo" >${map.wcmemo }</textarea>
						</div> 
					</div>
				</div>
				<div class="col-sm-12" >
					<div class=" form-group col-sm-12 ">
						<label  class="date-label col-sm-2">移交下一阶段</label>
						<div class="col-sm-4 ">
							<select  class="form-control col-sm-12" style="width:100%"
							 title="选择用户" 
							 null="no" id="nextexecuser" name="manageIntegral" >
								<option value=""></option>
						...
						<label  class="date-label col-sm-2">关闭任务</label>
						<div class="col-sm-4 ">
							<div class="form-control col-sm-8"  > 
							 <label class="radio-inline">
								<input type="radio"  value="1" name="isclose" id="isclose"  >是
							 </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							 <label class="radio-inline">
								<input type="radio"  value="0" name="isclose" id="isclose" checked >否
							 </label>	
							</div>
						</div>
				...
				<div class="col-sm-12 ">
					<div class="form-group col-sm-12">
						<label  class="date-label col-sm-2 ">移交信息</label>
						<div class="col-sm-10">
							<textarea rows="2" class="form-control" 
								null="no" id="yjinfo" name="yjinfo" >${map.yjinfo }</textarea>
						</div> 
					</div>
				</div>
		...
		<script type="text/javascript">
			...
			function setadd(aobj){
				...
				var serialno = "${map.serialno }";
				var taskid = "${map.taskid }";
				var executeuser = "${map.executeuser }";
				var wcmemo = $("#wcmemo").val();
				var nextexecuser = $("#nextexecuser").val();//下一阶段执行人
				var isclose = $('input[name="isclose"]:checked').val();
				var yjinfo = $("#yjinfo").val();
				var bjdstate = "";
				if(isclose == "1") {
					if(nextexecuser != "" && nextexecuser != null && nextexecuser != "null") {
						alert("关闭任务选择是，移交下一阶段必须为空");
						return;
					}
					bjdstate = "4";
				}
				...
				parent.layer.load(2); 
				$.ajax({
					url : '${pageContext.request.contextPath}/taskCl/taskHandle.action?rundd='+new Date().getTime(),
					type : 'post',
					data : {tablejson:str},
					dataType : "json",
					async : false,
